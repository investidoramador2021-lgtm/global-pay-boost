...
const AFFILIATE_WELCOME: Record<string, (name: string) => string> = {
  en: (n) => `Hi, I'm ${n}! I see you're checking out our **Partner Program** 🎯 — earn **commissions in BTC** every time someone swaps through your link or widget. Want me to walk you through generating your widget (it's translated into 13 languages — something no other affiliate widget offers) or help you register in under 60 seconds?`,
  es: (n) => `¡Hola, soy ${n}! Veo que está explorando nuestro **Programa de Socios** 🎯 — gane **comisiones en BTC** por cada intercambio referido. ¿Le ayudo a generar su widget (traducido a 13 idiomas — único en el mercado) o a registrarse en menos de 60 segundos?`,
  pt: (n) => `Olá, sou ${n}! Vejo que está explorando nosso **Programa de Parceiros** 🎯 — ganhe **comissões em BTC** a cada swap referido. Posso ajudar a gerar seu widget (traduzido em 13 idiomas — único no mercado) ou registrar em menos de 60 segundos?`,
  fr: (n) => `Bonjour, je suis ${n} ! Je vois que vous explorez notre **Programme Partenaire** 🎯 — gagnez des **commissions en BTC** sur chaque swap référé. Je vous aide à générer votre widget (traduit en 13 langues — unique sur le marché) ou à vous inscrire en moins de 60 secondes ?`,
  ja: (n) => `こんにちは、${n}です！**パートナープログラム**をご覧ですね 🎯 — 紹介スワップごとに **BTCでコミッション**を獲得できます。13言語に翻訳されたウィジェット（業界唯一）の生成、または60秒以内の登録をお手伝いしましょうか？`,
  tr: (n) => `Merhaba, ben ${n}! **Partner Programı**na göz attığınızı görüyorum 🎯 — yönlendirdiğiniz her takastan **BTC olarak komisyon** kazanın. 13 dile çevrilmiş widget'ınızı oluşturmanıza (sektörde tek) veya 60 saniyede kaydolmanıza yardımcı olayım mı?`,
  hi: (n) => `नमस्ते, मैं ${n} हूँ! आप हमारे **पार्टनर प्रोग्राम** को देख रहे हैं 🎯 — हर रेफर किए गए स्वैप पर **BTC में कमीशन** कमाएँ। 13 भाषाओं में अनुवादित विजेट जनरेट करने (इंडस्ट्री में अनोखा) या 60 सेकंड में रजिस्टर करने में मदद करूँ?`,
  vi: (n) => `Chào, tôi là ${n}! Bạn đang xem **Chương trình Đối tác** 🎯 — kiếm **hoa hồng bằng BTC** cho mỗi swap được giới thiệu. Tôi hỗ trợ bạn tạo widget (dịch ra 13 ngôn ngữ — duy nhất trên thị trường) hoặc đăng ký dưới 60 giây nhé?`,
  af: (n) => `Hallo, ek is ${n}! Ek sien jy kyk na ons **Vennoot Program** 🎯 — verdien **kommissies in BTC** vir elke verwysde ruil. Sal ek help om jou widget (vertaal in 13 tale — uniek in die mark) te genereer of jou binne 60 sekondes te registreer?`,
  fa: (n) => `سلام، من ${n} هستم! می‌بینم که **برنامه شرکا** را بررسی می‌کنید 🎯 — برای هر مبادله ارجاعی، **کمیسیون به BTC** کسب کنید. ویجت ترجمه‌شده به ۱۳ زبان (منحصربه‌فرد در بازار) را تولید کنیم یا در کمتر از ۶۰ ثانیه ثبت‌نام کنیم؟`,
  ur: (n) => `سلام، میں ${n} ہوں! آپ ہمارے **پارٹنر پروگرام** کو دیکھ رہے ہیں 🎯 — ہر ریفرڈ سواپ پر **BTC میں کمیشن** کمائیں۔ 13 زبانوں میں ترجمہ شدہ ویجٹ (مارکیٹ میں منفرد) بنانے یا 60 سیکنڈ میں رجسٹر ہونے میں مدد کروں؟`,
  he: (n) => `שלום, אני ${n}! אני רואה שאתה בודק את **תכנית השותפים** שלנו 🎯 — הרווח/י **עמלות ב-BTC** על כל המרה שתפנה/י. שאעזור לך ליצור את הווידג'ט (מתורגם ל-13 שפות — ייחודי בשוק) או להירשם תוך 60 שניות?`,
  uk: (n) => `Привіт, я ${n}! Бачу, ви розглядаєте нашу **Партнерську Програму** 🎯 — заробляйте **комісію у BTC** з кожного реферального обміну. Допомогти згенерувати ваш віджет (перекладений 13 мовами — єдиний на ринку) або зареєструватись за 60 секунд?`,
};
  ...
  const send = async () => {
    ...
    try {
      ...
      const resp = await fetch(CHAT_URL, {
        ...
        body: JSON.stringify({ messages: allMessages, persona: currentPersona.name, language: lang, pageUrl: location.pathname }),
      });
