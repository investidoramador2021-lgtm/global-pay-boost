  ...
  components: {
    ...
    schemas: {
      ...
      WebhookEvent: {
        type: "object",
        description:
          "Outbound webhook payload sent to the partner's `webhook_url`. " +
          "Delivered as `POST application/json` with headers `X-MRC-Event`, " +
          "`X-MRC-Signature` (hex HMAC-SHA256 of the raw body using `webhook_secret`), " +
          "and `X-MRC-Idempotency-Key` (`${order_id}:${event}:${state}`). " +
          "Receivers MUST de-duplicate on `idempotency_key`.",
        required: [
          "event",
          "order_id",
          "state",
        ...
        properties: {
          ...
          order_id: { type: "string", pattern: "^MRC-" },
          provider_order_id: { type: ["string", "null"] },
          state: { type: "string" },
          idempotency_key: {
            type: "string",
            description: "`${order_id}:${event}:${state}` — stable per delivery.",
          },
        ...
        examples: [
          {
            ...
            provider_order_id: "abc123def456",
            state: "waiting",
            idempotency_key: "MRC-1A2B3C4D-XY9Z:swap.created:waiting",
            occurred_at: "2026-04-23T15:32:00.000Z",
            from: "btc",
            to: "usdterc20",
            amount_in: "0.001",
            amount_out: "63.84",
          ...
          },
          {
            ...
            idempotency_key:
              "MRC-1A2B3C4D-XY9Z:swap.deposit_detected:confirming",
            occurred_at: "2026-04-23T15:42:09.000Z",
            from: "btc",
            to: "usdterc20",
            amount_in: "0.001",
            amount_out: "63.84",
          ...
          },
          {
            ...
            provider_order_id: "abc123def456",
            state: "finished",
            idempotency_key: "MRC-1A2B3C4D-XY9Z:swap.finished:finished",
            occurred_at: "2026-04-23T15:51:22.000Z",
            from: "btc",
            to: "usdterc20",
            amount_in: "0.001",
            amount_out: "63.91",
            ...
            payout_address: "0x742d35Cc6634C0532925a3b844Bc454e4438f44e",
            payout_hash:
              "0x9f3c1a2b4d5e6f708192a3b4c5d6e7f8091a2b3c4d5e6f708192a3b4c5d6e7f8",
          },
          {
            event: "swap.expired",
            order_id: "MRC-1A2B3C4D-XY9Z",
            provider_order_id: "abc123def456",
            state: "expired",
            idempotency_key: "MRC-1A2B3C4D-XY9Z:swap.expired:expired",
            occurred_at: "2026-04-23T16:02:00.000Z",
            from: "btc",
            to: "usdterc20",
            amount_in: "0.001",
            amount_out: null,
